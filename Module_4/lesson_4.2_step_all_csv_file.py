...
def sorting_column():
    with open("../files/deniro.csv", "r", encoding="utf-8") as csv_file:
        ...
        col = int(input()) - 1
        content = sorted(
            context, key=lambda x: int(x[col]) if x[col].isdigit() else x[col]
        )
...
def wifi():
    ...
    [print(f"{k}: {v}") for k, v in sorted(sorted(dic.items()), key=lambda x: -x[1])]
...
def student_counts():
    ...
    """
    with open("../files/student_counts.csv", "r", encoding="utf-8") as f, open(
